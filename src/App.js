...
const App = props => {
  ...
  useEffect(() => {
    ...
    axios
      .get(url, {
        headers: {
          Authorization: "Bearer " + props.id_token //the token is a variable which holds the token
        }
      })
      .then(response => {
  ...
  const refreshData = useCallback(() => {
    ...
    let progress = 0;
    let ordTrendStore = [];
    let enterpriseName = "Antonio Chinea";
    for (const row of items) {
      if (row.month === dropdownSelected) {
        ...
        progress += parseInt(row.abandoned_rate / 3, 10);
      }
    }
  ...
  return (
    <React.Fragment>
      <TopNav enterpriseName={enterpriseName} />
      <Nav
